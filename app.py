...
def export_pdf():
    ...
    story += [
        ...
        Paragraph("AI-Powered GRC Assessment · Powered by Google Gemini 2.5 Flash Preview",
            ps('Sub', fontSize=9, textColor=GREY,
               alignment=TA_CENTER, spaceAfter=12)),
        HRFlowable(width='100%', thickness=2, color=BLUE, spaceAfter=14),
    ]
    ...
    meta_rows = [
        [mcell('Framework', bold=True), mcell(data.get('framework_name', '')),
         mcell('Date',  bold=True),     mcell(data.get('analysis_date', ''))],
        [mcell('Document', bold=True),  mcell(data.get('document_name', '')),
    ...
    if action:
        ...
        for idx, c in enumerate(action):
            ...
            hdr  = Paragraph(
                f"<b>[{cid}]</b> {c.get('control_name','')[:80]} "
                f"— <font color='#{hex_}'>{pr.upper()}</font> ({stat})",
    ...
    story += [
        ...
        Paragraph(
            f"Generated by AI Compliance Gap Analyzer · "
            f"{data.get('analysis_date', '')} · Powered by Google Gemini 2.5 Flash Preview",
            ps('Ft', fontSize=7.5, textColor=GREY, alignment=TA_CENTER)),
    ]
